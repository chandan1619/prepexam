...
export default function StudyPage() {
  ...
  const [selectedPaidModule, setSelectedPaidModule] = useState<{title: string, index: number} | null>(null);
      ...
      try {
        ...
        try {
          const accessRes = await fetch(
            `/api/user/access?examId=${courseData.course.id}`
          );
  ...
  useEffect(() => {
    ...
    if (modules.length > 0 && !hasModuleAccessForInit(currentModuleIdx)) {
      ...
      const firstAccessibleModule = modules.findIndex((_: any, idx: number) =>
        hasModuleAccessForInit(idx)
      );
  ...
  // Toggle module expansion - only for accessible modules
  const toggleModuleExpansion = (moduleId: string, moduleIndex: number) => {
    // Check if user has access to this module
    if (!hasModuleAccess(moduleIndex)) {
      // Show custom payment modal for locked modules
      const moduleTitle = modules[moduleIndex]?.title || "Premium Module";
      setSelectedPaidModule({ title: moduleTitle, index: moduleIndex });
      setShowPaymentModal(true);
      return; // Don't expand locked modules
    }

    // Only expand if user has access
  ...
  // Handle WhatsApp redirect from payment modal
  const handleWhatsAppRedirect = () => {
    if (selectedPaidModule) {
      const message = encodeURIComponent(
        `Hi! I want to purchase the full course: "${course.title}". I'm trying to access the module "${selectedPaidModule.title}" which is locked. Please help me with the payment process. Course Price: ₹${course.priceInINR}. My email: ${user?.emailAddresses[0]?.emailAddress || "Not provided"}`
      );
      const whatsappUrl = `https://wa.me/918789449507?text=${message}`;
      window.open(whatsappUrl, "_blank");
    }
    setShowPaymentModal(false);
    setSelectedPaidModule(null);
  };
  ...
  function getLessons(module: {
    ...
    const lessons = [
      ...(module.blogPosts || []).map(
        (item): LessonType & { order: number } => ({
          type: "blogPost",
          ...item,
          order: item.order || 0,
        })
      ),
      ...(module.moduleQuestions || []).map(
        (item): LessonType & { order: number } => ({
          type: "question",
          id: item.id,
      ...
      ...(module.quizzes || []).map(
        (item): LessonType & { order: number; quizType?: string } => ({
          type: "quiz",
          id: item.id,
          title: item.title,
  ...
  return (
    <PageLayout>
      ...
      {currentLesson?.type === "quiz" &&
        (currentLesson as any)?.quizType === "ASSESSMENT" && (
          <QuizTimer
              ...
              return (
                <li key={mod.id} className="relative">
                  ...
                  <div className="relative">
                    <button
                      onClick={() => toggleModuleExpansion(mod.id, mIdx)}
                      className={`w-full font-semibold mb-3 pl-6 pr-2 py-2 flex items-center gap-2 rounded-lg transition-all duration-200 ${
                        isLocked
                ...
                course.priceInINR > 0 && (
                  <div className="flex-shrink-0">
                    <button
                      onClick={() => {
                        const message = encodeURIComponent(
                          `Hi! I want to purchase the full course: "${
                            course.title
                          }". I'm currently enrolled but want to unlock all premium modules. Course Price: ₹${
                            course.priceInINR
                          }. My email: ${
              ...
              <Card className="shadow-xl border-0 mb-8 overflow-hidden bg-white/90 backdrop-blur-sm hover:shadow-2xl transition-all duration-300">
                <CardHeader className="bg-gradient-to-r from-blue-50 to-purple-50 border-b">
                  <CardTitle className="flex items-center gap-3 text-xl">
                    ...
                    {currentLesson.type === "pyq" &&
                      `Previous Year Question (${currentLesson.year})`}
                  </CardTitle>
                </CardHeader>
                    ...
                    <div className="space-y-6">
                      <div className="p-6 bg-gradient-to-br from-purple-50 to-blue-50 rounded-xl border border-purple-100">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                            ...
                            {(currentLesson as any).pyqType === "boolean" &&
                              "True/False"}
                            {(currentLesson as any).pyqType === "descriptive" &&
                              "Descriptive"}
                          </div>
                        </div>
                        <div className="text-lg text-gray-900 leading-relaxed mb-4">
                          <div
                            dangerouslySetInnerHTML={{
                              __html: currentLesson.question.replace(
                                /^<p>|<\/p>$/g,
                                ""
                              ),
                            }}
                      ...
                      {/* MCQ Options */}
                      {(currentLesson as any).pyqType === "mcq" &&
                        (currentLesson as any).options && (
                          <div className="grid gap-3">
                            {(currentLesson as any).options.map(
                              (opt: string, idx: number) => (
                                <button
                                  key={idx}
                                  className={`w-full p-4 text-left rounded-xl border-2 transition-all duration-200 ${
                              ...
                              <div className="flex items-start gap-4">
                                <span
                                  className={`flex items-center justify-center w-8 h-8 rounded-lg text-sm font-semibold ${
                                    ...
                                    selectedOption === idx
                                      ? idx ===
                                        ((currentLesson as any).correct || 0)
                                        ? "bg-green-500 text-white"
                                        : "bg-red-500 text-white"
                                      : "bg-gray-100 text-gray-600"
                                  }`}
                                ...
                                <div className="text-sm">
                                  {(currentLesson as any).pyqType === "mcq" &&
                                    `The correct answer is: ${String.fromCharCode(
                                      65 + ((currentLesson as any).correct || 0)
                                    )}`}
                                  {(currentLesson as any).pyqType ===
                                    "boolean" &&
                                    `The correct answer is: ${
                                      ((currentLesson as any).correct || 0) ===
                                      0
            ...
            <div className="p-6">
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="text-lg font-bold text-gray-900">Premium Content</h3>
              ...
              <div className="mb-6">
                <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-xl p-4 mb-4">
                  <h4 className="font-semibold text-gray-900 mb-2">
                    "{selectedPaidModule?.title}"
                  </h4>
                  <p className="text-sm text-gray-600">
                    This is a premium module. To access this content, you need to purchase the full course.
                  </p>
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center justify-between py-2 border-b border-gray-100">
                    ...
                    <span className="font-bold text-2xl text-green-600">₹{course?.priceInINR}</span>
                  </div>
              ...
              <div className="flex gap-3">
                ...
                <button
                  ...
                  className="flex-1 px-4 py-3 bg-gradient-to-r from-green-500 to-emerald-500 text-white rounded-xl hover:shadow-lg hover:shadow-green-500/25 transition-all duration-200 font-medium flex items-center justify-center gap-2"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="h-5 w-5"
