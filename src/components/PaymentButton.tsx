...
export default function PaymentButton({
  ...
  const handlePayment = async () => {
    ...
    const message = encodeURIComponent(
      `Hi! I want to purchase the full course: "${examTitle}". Please help me with the payment process. Course Price: ₹499 (50% OFF from ₹999). My email: ${user?.emailAddresses[0]?.emailAddress || 'Not provided'}`
    );
    ...
    >
      {loading ? (
        ...
        <div className="flex items-center justify-center gap-2 cursor-pointer">
          ...
          Start Learning (Free) - Contact for Premium ₹499 (50% OFF)
        </div>
      )}
    </button>
  );
